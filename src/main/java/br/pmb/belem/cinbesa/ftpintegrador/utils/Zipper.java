...
public class Zipper {
    ...
    public static void main(String[] args) throws IOException {
        ...
        File fileZip = new File("F:/copia/backup20110818_21-09.zip" );
        
        List<ZipEntry> listaArquivos = zip.listarEntradasZip(fileZip);
        for (ZipEntry zipEntry : listaArquivos) {
            System.out.println("arquivo="+zipEntry.getName());
        }
 
        
        File diretorio = new File("C:/temp/copia2/");  // cria o diretório, caso não exista.
        zip.setArquivoZipAtual(fileZip);
        zip.extrairZip( diretorio);
        zip.fecharZip();
